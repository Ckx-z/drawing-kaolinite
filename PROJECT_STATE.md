...
> 最后更新：2026-09-05（TODO 清单落档 + Q1–Q4 决策闭环 + 记忆系统骨架 T-8.1 完成）
...
| **阶段 1（当前）** | P0 工程基座：T-1.1 ~ T-1.7（Vite+TS+React 脚手架 → Schema → 内核 TS 移植 → 渲染服务 → 状态管理 → UI 迁移 → 回归验收） | ⬜ **下一步** |
...
1. **T-1.1 Vite + TypeScript + React 工程脚手架**（关键路径起点，约 1d）——`src/` 目录按 `core/ render/ state/ ui/ export/` 划分，`demo/` 保留为回归基线
2. T-1.2 数据模型与 zod Schema（依赖 T-1.1）
3. T-1.3 几何内核 TS 移植 + 单测（断言对齐 `demo/core/test.js` 基线数值）
4. T-8.2 `check_library_state.py` 对账脚本（P2，可与阶段 1 并行）
5. T-9.1 / T-9.2 首批素材模块（阶段 1 完成后立即开始）
