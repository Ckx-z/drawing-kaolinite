...
> 最后更新：2026-09-06（T-3.1 组合模块完成，阶段 2 已完成 5 条；下一步 T-2.2 Worker 化 / T-9.3·T-9.4 素材模块）
...
| **阶段 2（当前）** | P1：T-2.1 撤销重做 / T-2.2 Worker 化 / T-2.3 IndexedDB 模块库 / T-3.1 组合模块 / T-9.x 素材积累 | 🔶 **进行中**（T-2.1 ✅、T-2.3 ✅、T-3.1 ✅、T-9.1 ✅、T-9.2 ✅；素材库已有 3 个模块） |
...
1. **T-2.2 Web Worker 化几何生成**（依赖 T-1.3 ✅，约 1d）——大场景参数拖动不卡 UI（T-2.9 缓存的前置）
2. **T-9.3 / T-9.4 素材模块**（M4 CeO₂ 颗粒组 / M5 橡胶基底，各 0.5d）——T-9.5 M6 组合模块（依赖 T-3.1 ✅）可随后入库
3. **T-3.2 模块检索/分类/版本**（依赖 T-2.3 ✅，约 1d）
