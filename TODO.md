...
- [ ] **T-11.6 [P1] 纯 2D 示意图模式**（1d）顶栏模式切换；隐藏 3D + view pan/zoom 无限画布 + 格点背景；导出复用
- [ ] **T-11.7 [P1] 自动对齐吸附**（1d）网格吸附开关 + 边缘/中心对齐参考线（阈值 6px 洋红线；可与 T-7.3 共用算法）
- [ ] **T-11.8 [P1] 机理图模板库**（1d，吸收 T-9.8）模板=场景子集（components?+shapes+annotations）；种子 4 版式（界面反应/三步流程/循环/对比）；Dexie 自存模板
