...
npm run test       # vitest 551 条（几何 / 矿物 / 分子显示模型 / 表面与吸附工作流 / 测量 / 状态 / 导出 / SMILES）
...
- **纯 2D 示意图模式** + 磁吸对齐（六线 6px + 网格）+ 机理图模板库（4 种子版式 + 自存模板）；**图元 Ctrl/Cmd+C / V / D 复制粘贴**（五类图元、连续粘贴逐次错开、锚定副本自动转自由图元）；**所有分子组件统一球棍/空间填充切换**（preset/SMILES/化学式/MOL/SDF/吸附应用同权）
