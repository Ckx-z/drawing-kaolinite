...
| 种子模板库 | `data/seed-modules.json` + `data/seed-templates.json` | M2~M14 共 15 个成品模块 + 4 种子版式模板，统一进入「模板」Tab，「导入」一键恢复 |
...
npm run test       # vitest 603 条（几何 / 矿物 / 分子 / 资产搜索与直加 / 表面缺陷与吸附 / 测量 / 状态 / 导出 / SMILES）
...
- **八种矿物 CIF 直驱**：高岭石 / 地开石 / 珍珠石 / 蒙脱石 / 伊利石 / 莫来石 / 四氧化三钴（尖晶石）/ 二氧化铈（萤石）——中文名搜索直达，d001 随矿物自动联动，原子数与晶胞化学计量严格一致（莫来石为 Pbam 骨架结构 COD 2310785，堆叠周期 = c 轴）
...
- **纯 2D 示意图模式** + 磁吸对齐（六线 6px + 网格）+ 机理图模板库（4 种子版式 + 自存模板）；**图元 Ctrl/Cmd+C / V / D 复制粘贴**（五类图元、连续粘贴逐次错开、锚定副本自动转自由图元）；**所有分子组件统一球棍/空间填充切换**（preset/SMILES/化学式/MOL/SDF/吸附应用同权）；**拖拽贴附**——拖动分子与片层/管/颗粒/表面重叠松手即自动吸附贴合（一条 Ctrl+Z 解除；按住 Alt/Shift 自由放置）；**表面氧空位**——选中晶体表面 →「表面缺陷」拾取表层 O → 创建 CeO₂(111)-Ov（未弛豫几何缺陷，可吸附建模，Ctrl+Z 恢复）
- **多选批量**：Shift+点击多选 → 对齐 / 等距分布 / 统一参数风格
- **键长/键角测量**：Alt+点击原子——两点键长（Å）、第三点键角（°），随 PNG/SVG 导出
- **无限连续旋转**（多圈无跳变）+ gizmo 吸附（1/2/5Å、5°/15°/45°）

**统一模板库**（2026-09-17）：moduleLibrary 底座吸收模板快照能力——单组件模块/组合模块/完整画面模板（组件+图元+相机+2D 视图+**真实整景缩略图**）同处「我的模板」，**点击 = 作为独立场景打开**（一次性全量替换，不叠加，✎ 可重命名）
> **表面与吸附工作流**：素材库「晶体表面」→ CeO₂(111) 等 Miller 切割 → 参数面板「吸附构型」选甲苯/丙烷 → 生成 4 候选（单预览切换、碰撞自动修正）→ 应用为正式分子组件 → 存模板完整复现。候选为未优化初始几何（诚实标注，无能量计算）；IndexedDB 持久化（200 条 <100ms）、关键词/类型检索、收藏置顶、`.kaolin-modules.json` 导入导出、旧模板库自动无损迁移
