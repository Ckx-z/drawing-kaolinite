...
npm run test       # vitest 566 条（几何 / 矿物 / 分子 / 表面吸附与拖拽贴附 / 误触审计 / 测量 / 状态 / 导出 / SMILES）
...
- **纯 2D 示意图模式** + 磁吸对齐（六线 6px + 网格）+ 机理图模板库（4 种子版式 + 自存模板）；**图元 Ctrl/Cmd+C / V / D 复制粘贴**（五类图元、连续粘贴逐次错开、锚定副本自动转自由图元）；**所有分子组件统一球棍/空间填充切换**（preset/SMILES/化学式/MOL/SDF/吸附应用同权）；**拖拽贴附**——拖动分子与片层/管/颗粒/表面重叠松手即自动吸附贴合（一条 Ctrl+Z 解除；按住 Alt/Shift 自由放置）
